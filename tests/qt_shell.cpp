...
int main(int argc, char **argv)
{
    ...
    if (hoverRow) {
        ...
        QTest::mouseMove(window, QPoint(30,300));
        QTest::qWait(550);
        ok &= check(window->property("activeTheme").toString() == "tinted-catppuccin-latte", "Leaving row cancels pending preview");
    }
    QTest::qWait(260);
    ...
    ok &= check(IsZoomed(reinterpret_cast<HWND>(window->winId())), "Windows reports a maximized window");
#else
    ...
    ok &= click("maximizeButton");
    for (int wait = 0; wait < 20 && window->visibility() != QWindow::Windowed; ++wait) QTest::qWait(100);
    ok &= check(window->visibility() == QWindow::Windowed, "Header restores window");

...
Window {
    ...
    ColumnLayout {
        ...
        Label {
            ...
            font.family: hudTheme.family; font.pixelSize: 23 * hudTheme.scale; color: hudTheme.text
        }
        Label {
        ...
        RowLayout {
            visible: hud.showControls; enabled: hud.controlsEnabled
            Layout.fillWidth: true; spacing: 4 * hudTheme.scale
            QuietButton { theme: hudTheme; text: "‹"; Accessible.name: "Previous step"; subtle: true; transparentIdle: true; focusPolicy: Qt.NoFocus; visible: !!hud.state.sequence; enabled: hud.bridge && !hud.bridge.dirty; onClicked: hud.bridge.execute(4) }
            QuietButton { theme: hudTheme; text: "Reset"; subtle: true; transparentIdle: true; focusPolicy: Qt.NoFocus; visible: !!hud.state.sequence; enabled: hud.bridge && !hud.bridge.dirty; onClicked: hud.bridge.execute(5) }
            QuietButton { objectName: "hudNext"; theme: hudTheme; text: "›"; Accessible.name: "Next step"; subtle: true; transparentIdle: true; focusPolicy: Qt.NoFocus; visible: !!hud.state.sequence; enabled: hud.bridge && !hud.bridge.dirty; onClicked: hud.bridge.execute(3) }
            Item { Layout.fillWidth: true }
            QuietButton { objectName: "hudToggle"; theme: hudTheme; text: hud.state.running ? "Stop" : "Start"; focusPolicy: Qt.NoFocus; enabled: hud.bridge && (hud.state.running || hud.state.canStart); onClicked: hud.bridge.execute(2) }
        }
    }
}
